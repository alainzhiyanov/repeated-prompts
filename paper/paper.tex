...
point, still gains $+5.6$, $+5.1$, and $+5.1$ pp. The total gain over
the vanilla single-prompt baseline (condition 3 $-$ condition 1) is
uniformly positive on the in-distribution benchmarks except for
Mistral-7B's GSM8K, and is much smaller on the held-out benchmarks
(see the per-model tables in \cref{app:numbers}).
...
All training scripts, evaluation pipelines, and per-benchmark results
are available at
\url{https://github.com/alainzhiyanov/repeated-prompts}.
...
underlying \cref{fig:main,fig:gain}.
